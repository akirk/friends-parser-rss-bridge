...
{
	if($maxLen <= 0) { $maxLen = simple_html_dom_const::MAX_FILE_SIZE; }
...
{
	public $nodetype = simple_html_dom_const::HDOM_TYPE_TEXT;
	public $tag = 'text';
	public $attr = array();
	...
	function dump_node($echo = true)
	{
		...
		$string .= ' simple_html_dom_const::HDOM_INNER_INFO: ';

		if (isset($node->_[simple_html_dom_const::HDOM_INFO_INNER])) {
			$string .= "'" . $node->_[simple_html_dom_const::HDOM_INFO_INNER] . "'";
		} else {
			$string .= ' NULL ';
		}
	...
	function innertext()
	{
		if (isset($this->_[simple_html_dom_const::HDOM_INFO_INNER])) {
			return $this->_[simple_html_dom_const::HDOM_INFO_INNER];
		}

		if (isset($this->_[simple_html_dom_const::HDOM_INFO_TEXT])) {
			return $this->dom->restore_noise($this->_[simple_html_dom_const::HDOM_INFO_TEXT]);
		}
	...
	function outertext()
	{
		...
		if (isset($this->_[simple_html_dom_const::HDOM_INFO_OUTER])) {
			return $this->_[simple_html_dom_const::HDOM_INFO_OUTER];
		}
		...
		if ($this->dom && $this->dom->nodes[$this->_[simple_html_dom_const::HDOM_INFO_BEGIN]]) {
			$ret = $this->dom->nodes[$this->_[simple_html_dom_const::HDOM_INFO_BEGIN]]->makeup();
		}

		if (isset($this->_[simple_html_dom_const::HDOM_INFO_INNER])) {
			// todo: <br> should either never have simple_html_dom_const::HDOM_INFO_INNER or always
			if ($this->tag !== 'br') {
				$ret .= $this->_[simple_html_dom_const::HDOM_INFO_INNER];
			}
		} elseif ($this->nodes) {
			foreach ($this->nodes as $n) {
		...
		if (isset($this->_[simple_html_dom_const::HDOM_INFO_END]) && $this->_[simple_html_dom_const::HDOM_INFO_END] != 0) {
			$ret .= '</' . $this->tag . '>';
		}
	...
	function text()
	{
		...
		switch ($this->nodetype) {
			case simple_html_dom_const::HDOM_TYPE_TEXT: return $this->dom->restore_noise($this->_[simple_html_dom_const::HDOM_INFO_TEXT]);
			case simple_html_dom_const::HDOM_TYPE_COMMENT: return '';
			case simple_html_dom_const::HDOM_TYPE_UNKNOWN: return '';
		}
		...
		// In rare cases, (always node type 1 or simple_html_dom_const::HDOM_TYPE_ELEMENT - observed
	...
	function makeup()
	{
		...
		foreach ($this->attr as $key => $val) {
			...
			$ret .= $this->_[simple_html_dom_const::HDOM_INFO_SPACE][$i][0];

			//no value attr: nowrap, checked selected...
			if ($val === true) {
				$ret .= $key;
			} else {
				switch ($this->_[simple_html_dom_const::HDOM_INFO_QUOTE][$i])
				{
					case simple_html_dom_const::HDOM_QUOTE_DOUBLE: $quote = '"'; break;
					case simple_html_dom_const::HDOM_QUOTE_SINGLE: $quote = '\''; break;
					default: $quote = '';
				}

				$ret .= $key
				. $this->_[simple_html_dom_const::HDOM_INFO_SPACE][$i][1]
				. '='
				. $this->_[simple_html_dom_const::HDOM_INFO_SPACE][$i][2]
				. $quote
				. $val
				. $quote;
			}
		...
		return $ret . $this->_[simple_html_dom_const::HDOM_INFO_ENDSPACE] . '>';
	}
	...
	function find($selector, $idx = null, $lowercase = false)
	{
		...
		for ($c = 0; $c < $count; ++$c) {
			...
			if (!isset($this->_[simple_html_dom_const::HDOM_INFO_BEGIN])) { return array(); }

			$head = array($this->_[simple_html_dom_const::HDOM_INFO_BEGIN] => 1);
			$cmd = ' '; // Combinator
	...
	protected function seek($selector, &$ret, $parent_cmd, $lowercase = false)
	{
		...
		if ($parent_cmd === ' ') { // Descendant Combinator
			...
			$end = (!empty($this->_[simple_html_dom_const::HDOM_INFO_END])) ? $this->_[simple_html_dom_const::HDOM_INFO_END] : 0;
			if ($end == 0) {
				$parent = $this->parent;
				while (!isset($parent->_[simple_html_dom_const::HDOM_INFO_END]) && $parent !== null) {
					$end -= 1;
					$parent = $parent->parent;
				}
				$end += $parent->_[simple_html_dom_const::HDOM_INFO_END];
			}

			// Get list of target nodes
			$nodes_start = $this->_[simple_html_dom_const::HDOM_INFO_BEGIN] + 1;
		...
		foreach($nodes as $node) {
			...
			if ($pass) $ret[$node->_[simple_html_dom_const::HDOM_INFO_BEGIN]] = 1;
			unset($node);
		}
	...
	function __set($name, $value)
	{
		...
		switch ($name) {
			case 'outertext': return $this->_[simple_html_dom_const::HDOM_INFO_OUTER] = $value;
			case 'innertext':
				if (isset($this->_[simple_html_dom_const::HDOM_INFO_TEXT])) {
					return $this->_[simple_html_dom_const::HDOM_INFO_TEXT] = $value;
				}
				return $this->_[simple_html_dom_const::HDOM_INFO_INNER] = $value;
		}

		if (!isset($this->attr[$name])) {
			$this->_[simple_html_dom_const::HDOM_INFO_SPACE][] = array(' ', '', '');
			$this->_[simple_html_dom_const::HDOM_INFO_QUOTE][] = simple_html_dom_const::HDOM_QUOTE_DOUBLE;
		}
...
class simple_html_dom
{
		...
		$options = 0)
	{
		...
		if($options & simple_html_dom_const::HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
		...
		$this->parse();
		// end
		$this->root->_[simple_html_dom_const::HDOM_INFO_END] = $this->cursor;
		$this->parse_charset();
		...
		$defaultSpanText = simple_html_dom_const::DEFAULT_SPAN_TEXT)
	{
		...
		$this->root->tag = 'root';
		$this->root->_[simple_html_dom_const::HDOM_INFO_BEGIN] = -1;
		$this->root->nodetype = simple_html_dom_const::HDOM_TYPE_ROOT;
	...
	protected function parse()
	{
		while (true) {
			...
			++$this->cursor;
			$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = $s;
	...
	protected function read_tag()
	{
		...
		if ($this->char === '/') {
			...
			if ($parent_lower !== $tag_lower) {
				...
				if (isset($this->optional_closing_tags[$parent_lower])
					...
					$this->parent->_[simple_html_dom_const::HDOM_INFO_END] = 0;
					...
					if (strtolower($this->parent->tag) !== $tag_lower) {
						...
						$this->parent->_[simple_html_dom_const::HDOM_INFO_END] = $this->cursor;
					...
					$this->parent->_[simple_html_dom_const::HDOM_INFO_END] = 0; // No end tag
					$org_parent = $this->parent;
		...
		// start tag
		$node = new simple_html_dom_node($this);
		$node->_[simple_html_dom_const::HDOM_INFO_BEGIN] = $this->cursor;
		++$this->cursor;
		...
		if (isset($tag[0]) && $tag[0] === '!') {
			$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

			if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
				$node->nodetype = simple_html_dom_const::HDOM_TYPE_COMMENT;
				$node->tag = 'comment';
			} else { // Could be doctype or CDATA but we don't care
				$node->nodetype = simple_html_dom_const::HDOM_TYPE_UNKNOWN;
				$node->tag = 'unknown';
			}

			if ($this->char === '>') { $node->_[simple_html_dom_const::HDOM_INFO_TEXT] .= '>'; }
		...
		if ($pos = strpos($tag, '<') !== false) {
			$tag = '<' . substr($tag, 0, -1);
			$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = $tag;
			$this->link_nodes($node, false);
		...
		if (!preg_match('/^\w[\w:-]*$/', $tag)) {
			$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
		...
		$node->nodetype = simple_html_dom_const::HDOM_TYPE_ELEMENT;
		$tag_lower = strtolower($tag);
		...
		do {
			...
			if ($this->pos >= $this->size - 1 && $this->char !== '>') {
				$node->nodetype = simple_html_dom_const::HDOM_TYPE_TEXT;
				$node->_[simple_html_dom_const::HDOM_INFO_END] = 0;
				$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
				$node->tag = 'text';
				$this->link_nodes($node, false);
				return true;
			}
			...
			if ($this->doc[$this->pos - 1] == '<') {
				...
				$node->attr = array();
				$node->_[simple_html_dom_const::HDOM_INFO_END] = 0;
				$node->_[simple_html_dom_const::HDOM_INFO_TEXT] = substr(
					$this->doc,
					$begin_tag_pos,
			...
			if ($name !== '/' && $name !== '') { // this is a attribute name
				...
				if ($this->char === '=') { // attribute with value
					...
					//no value attr: nowrap, checked selected...
					$node->_[simple_html_dom_const::HDOM_INFO_QUOTE][] = simple_html_dom_const::HDOM_QUOTE_NO;
					$node->attr[$name] = true;
					if ($this->char != '>') { $this->char = $this->doc[--$this->pos]; } // prev
				}

				$node->_[simple_html_dom_const::HDOM_INFO_SPACE][] = $space;
		...
		$this->link_nodes($node, true);
		$node->_[simple_html_dom_const::HDOM_INFO_ENDSPACE] = $space[0];

		// handle empty tags (i.e. "<div/>")
		if ($this->copy_until_char('>') === '/') {
			$node->_[simple_html_dom_const::HDOM_INFO_ENDSPACE] .= '/';
			$node->_[simple_html_dom_const::HDOM_INFO_END] = 0;
		} else {
			// reset parent
		...
		if ($node->tag === 'br') {
			$node->_[simple_html_dom_const::HDOM_INFO_INNER] = $this->default_br_text;
		}
	...
	protected function parse_attr($node, $name, &$space)
	{
		...
		switch ($this->char) {
			case '"':
				$quote_type = simple_html_dom_const::HDOM_QUOTE_DOUBLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('"');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			case '\'':
				$quote_type = simple_html_dom_const::HDOM_QUOTE_SINGLE;
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				$value = $this->copy_until_char('\'');
				$this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
				break;
			default:
				$quote_type = simple_html_dom_const::HDOM_QUOTE_NO;
		...
		if (!$is_duplicate) {
			$node->_[simple_html_dom_const::HDOM_INFO_QUOTE][] = $quote_type;
